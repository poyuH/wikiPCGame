...
def transaction_history():
    conn = my_db.get_conn()
    cursor = conn.execute("SELECT tid FROM Attend_Transaction T WHERE T.account='%s'" % g.user)
    name_urls = []
    for result in cursor:
        name_urls.append((result[Transaction.TID.value], '/transaction_history/' + quote(result[Transaction.TID.value])))
    cursor.close()
    context = {}
